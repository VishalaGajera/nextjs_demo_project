...
import { useEditSetupConfigureLeave } from "../../../../../../../../components/PolicyConfiguration/Leave/LeavePlan/ConfigureLeave/hooks/useEditSetupConfigureLeave";
...
export default function Page({ params }: PageProps) {
  ...
  const { mutate, isPending } = useEditSetupConfigureLeave({
    leavePlanId,
    leaveTypeId,
    options: {
      onSuccess: (data) => {
  ...
  const handleFormSteps = (step: string) => {
    if (isEmpty(formRef.current?.errors)) {
      return router.push(
        `/policy-configuration/leave/leave-plan/${leavePlanId}/configure-leave/edit/${leaveTypeId}?step=${step}`
      );
    }
  };
