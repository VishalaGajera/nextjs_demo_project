...
import { useGetSkillTypeHistoryColumns } from "./hooks/useGetSkillTypeColumns";

type SkillTypeHistoryListProps = {
  employeeId: string;
};

export const SkillTypeHistoryList = ({
  employeeId,
}: SkillTypeHistoryListProps) => {
  const gridRef = useRef<AgGridReact<Histories>>(null);

  const [loading, setLoading] = useState(false);

  const { column } = useGetSkillTypeHistoryColumns({ loading });
