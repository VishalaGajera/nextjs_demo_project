...
import { useLoanPolicyHistoryColumns } from "./hooks/useLoanPolicyHistoryColumns";

type LoanPolicyHistoryListProps = {
  employeeId: string;
};

export const LoanPolicyHistoryList = ({
  employeeId,
}: LoanPolicyHistoryListProps) => {
  const gridRef = useRef<AgGridReact<Histories>>(null);

  const [loading, setLoading] = useState(false);

  const { column } = useLoanPolicyHistoryColumns({ loading });
