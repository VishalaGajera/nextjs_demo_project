...
import { useGetOvertimeRuleHistoryColumns } from "./hooks/useGetOvertimeRuleHistoryColumns";

type OvertimeRuleHistoryListProps = {
  employeeId: string;
};

export const OvertimeRuleHistoryList = ({
  employeeId,
}: OvertimeRuleHistoryListProps) => {
  const gridRef = useRef<AgGridReact<Histories>>(null);

  const [loading, setLoading] = useState(false);

  const { column } = useGetOvertimeRuleHistoryColumns({ loading });
