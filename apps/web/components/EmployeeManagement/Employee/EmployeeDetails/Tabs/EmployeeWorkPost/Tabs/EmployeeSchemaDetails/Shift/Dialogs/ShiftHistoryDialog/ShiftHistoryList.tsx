...
import { useGetShiftHistoryColumns } from "./hooks/useGetShiftHistoryColumns";

type ShiftHistoryListProps = {
  employeeId: string;
};

export const ShiftHistoryList = ({ employeeId }: ShiftHistoryListProps) => {
  const gridRef = useRef<AgGridReact<Histories>>(null);

  const [loading, setLoading] = useState(false);

  const { column } = useGetShiftHistoryColumns({ loading });
