...
import { useGetLeavePlanHistoryColumns } from "./hooks/useGetLeavePlanHistoryColumns";

type LeavePlanHistoryListProps = {
  employeeId: string;
};

export const LeavePlanHistoryList = ({
  employeeId,
}: LeavePlanHistoryListProps) => {
  const gridRef = useRef<AgGridReact<Histories>>(null);

  const [loading, setLoading] = useState(false);

  const { column } = useGetLeavePlanHistoryColumns({ loading });
