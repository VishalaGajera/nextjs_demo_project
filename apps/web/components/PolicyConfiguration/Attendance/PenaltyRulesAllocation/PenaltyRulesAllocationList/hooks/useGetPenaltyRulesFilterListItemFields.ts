...
export const useGetFilterListItemField = ({
  ...
  const { data: penaltyRulesOptions } = usePenaltyRuleOptions({
  ...
  const filterListItemsField: FilterListType[] = useMemo(
    () => [
      ...
      },
      {
        ...
        key: "attendance_penalty_rule_id",
        value: currentFilter?.attendance_penalty_rule_id,
        multiSelect: true,
        type: "autoComplete",
        options: penaltyRulesOptions,
        isDisabled: !currentFilter?.company_id,
      },
    ],
