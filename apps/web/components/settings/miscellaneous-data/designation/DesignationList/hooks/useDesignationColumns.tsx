...
import type { Designation } from "./useDesignations";

type UseDesignationColumns = {
  onAction: (actionType: DialogTypes, rowData: Designation) => void;
  onActive: (rowData?: Designation) => void;
};

export const useDesignationColumns = ({
  onAction,
  loading,
  onActive,
}: AgColumnsArgs<UseDesignationColumns>) => {
  const columns: AgColumnsWithActions<Designation> = [
    {
    ...
    },
    {
      ...
      filter: "agTextColumnFilter",
      sortable: true,
      cellRenderer: ({ data }: CustomCellRendererProps<Designation>) => {
        if (loading) {
          return <LoadingCell />;
        }
