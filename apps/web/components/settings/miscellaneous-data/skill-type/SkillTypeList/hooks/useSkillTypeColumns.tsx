...
import type { SkillType } from "./useGetSkillTypes";

type UseSkillTypeColumnsArgs = {
  onAction: (actionType: DialogTypes, rowData: SkillType) => void;
  onActive: (rowData?: SkillType) => void;
};

export const useSkillTypeColumns = ({
  onAction,
  loading,
  onActive,
}: AgColumnsArgs<UseSkillTypeColumnsArgs>) => {
  const columns: AgColumnsWithActions<SkillType> = [
    {
    ...
    {
      headerName: "Skill Type Code",
      field: "skill_type_code",
      cellRenderer: ({ value }: CustomCellRendererProps) => {
        if (loading) {
          return <LoadingCell />;
        }
    ...
    },
    {
      ...
      filter: "agTextColumnFilter",
      sortable: true,
      cellRenderer: ({ data }: CustomCellRendererProps<SkillType>) => {
        if (loading) {
          return <LoadingCell />;
        }
