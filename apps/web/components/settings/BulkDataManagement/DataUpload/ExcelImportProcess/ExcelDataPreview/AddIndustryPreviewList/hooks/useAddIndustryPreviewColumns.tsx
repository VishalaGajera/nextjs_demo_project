...
import IndustryFormSchema from "../config.json";

type UseAddIndustryPreviewColumnsArgs = {
  excelRowsData: ExcelRowsData;
  excelMappedOptions: ExcelMappedOptions;
};

export const schema = createDynamicZodSchema(
  IndustryFormSchema as ValidationSchema,
  INDUSTRY
);

export type IndustryFormFieldValues = z.infer<typeof schema>;

export const useAddIndustryPreviewColumns = ({
  excelRowsData,
  excelMappedOptions,
}: UseAddIndustryPreviewColumnsArgs) => {
  const { t } = useTranslation();

  const form = useForm<IndustryFormFieldValues>({
    values: { [INDUSTRY]: excelRowsData as [{ [key: string]: unknown }] },
  ...
  const defaultColumnField = {
    ...
    cellRendererParams: {
      control,
      masterCodeKey: INDUSTRY,
    },
  };

  const getError = (index: number, fieldName: string) => {
    return errors[INDUSTRY]?.[index]?.[fieldName];
  };
