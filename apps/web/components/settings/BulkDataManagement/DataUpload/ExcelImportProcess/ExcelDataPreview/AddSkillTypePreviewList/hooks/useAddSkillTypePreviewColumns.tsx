...
import SkillTypeFormSchema from "../config.json";

type UseAddSkillTypePreviewColumnsArgs = {
  excelRowsData: ExcelRowsData;
  excelMappedOptions: ExcelMappedOptions;
};

export const schema = createDynamicZodSchema(
  SkillTypeFormSchema as ValidationSchema,
  SKILL_TYPE
);

export type SkillTypeFormFieldValues = z.infer<typeof schema>;

export const useAddSkillTypePreviewColumns = ({
  excelRowsData,
  excelMappedOptions,
}: UseAddSkillTypePreviewColumnsArgs) => {
  const { t } = useTranslation();

  const form = useForm<SkillTypeFormFieldValues>({
    values: { [SKILL_TYPE]: excelRowsData as [{ [key: string]: unknown }] },
  ...
  const getError = (index: number, fieldName: string) => {
    return errors[SKILL_TYPE]?.[index]?.[fieldName];
  };
