...
import DepartmentFormSchema from "../config.json";

type UseAddDepartmentPreviewColumnsArgs = {
  excelRowsData: ExcelRowsData;
  excelMappedOptions: ExcelMappedOptions;
};

export const schema = createDynamicZodSchema(
  DepartmentFormSchema as ValidationSchema,
  DEPARTMENT
);

export type DepartmentFormFieldValues = z.infer<typeof schema>;

export const useAddDepartmentPreviewColumns = ({
  excelRowsData,
  excelMappedOptions,
}: UseAddDepartmentPreviewColumnsArgs) => {
  const { t } = useTranslation();

  const form = useForm<DepartmentFormFieldValues>({
    values: { [DEPARTMENT]: excelRowsData as [{ [key: string]: unknown }] },
  ...
  const getError = (index: number, fieldName: string) => {
    return errors[DEPARTMENT]?.[index]?.[fieldName];
  };
